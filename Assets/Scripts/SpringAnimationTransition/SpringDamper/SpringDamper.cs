...
namespace SpringDamperSystem
{
    ...
    public class SpringDamper
    {
        ...
        static void spring_damper_exact(
            float x,
            float v,
            float x_goal,
            float v_goal,
        ...
        /// <param name="x_goal">目标值</param>
        /// <param name="halflife">半衰期</param>
        /// <param name="dt">时间增量</param>
        public static void SimpleSpringDamperExactQuat(
            ref Quaternion x,
            ref Vector3 v,
            Quaternion x_goal,
            float halflife,
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public static void SpringDamperExactVector(
            ref Vector3 x,
            ref Vector3 v,
            Vector3 x_goal,
            Vector3 v_goal,
            ...
            float eps = 1e-5f)
        {
            float tempX, tempV;
            tempX = x.x; tempV = v.x;
            SpringDamperExactRatio(ref tempX, ref tempV, x_goal.x, v_goal.x, damping_ratio, halflife, dt, eps);
            x.x = tempX; v.x = tempV;

            tempX = x.y; tempV = v.y;
            SpringDamperExactRatio(ref tempX, ref tempV, x_goal.y, v_goal.y, damping_ratio, halflife, dt, eps);
            x.y = tempX; v.y = tempV;

            tempX = x.z; tempV = v.z;
            SpringDamperExactRatio(ref tempX, ref tempV, x_goal.z, v_goal.z, damping_ratio, halflife, dt, eps);
            x.z = tempX; v.z = tempV;
        }
    }
}
